...
function EventSection() {
  ...
  useEffect(() => {
    fetch("https://clubsandsocs.jakefarrell.ie/mulife.ie/society/esn/events")
  ...
  return (
    ...
    {/* Event details */}
    <div>
    <h2 className="text-xl font-bold mb-2">{data.events.event_0.name}</h2>
    <p className="text-gray-600 text-sm mb-2">Start Time: {data.events.event_0.start}</p>
    <p className="text-gray-600 text-sm mb-2">End Time: {data.events.event_0.end}</p>
    <p className="text-gray-600 text-sm mb-2">Location: {data.events.event_0.location}</p>
    </div>
    {/* Modal trigger button */}
    <button
      className="bg-blue-500 text-white px-4 py-2 mt-7 rounded-lg hover:bg-blue-600 transition-colors"
      onClick={() => openModal(data.events.event_0.name, data.events.event_0.description)}
    >
      More Info
    </button>
    </div>
    ...
    <p className="text-gray-600 text-sm mb-2">Start Time: 10:00 AM</p>
    <p className="text-gray-600 text-sm mb-2">End Time: 12:00 PM</p>
    <p className="text-gray-600 text-sm mb-2">Location: XYZ Hall</p>
    </div>
    {/* Modal trigger button */}
    <button
    className="bg-blue-500 text-white px-4 py-2 mt-7 rounded-lg hover:bg-blue-600 transition-colors"
    onClick={() => openModal("Event 1", "Lorem ipsum dolor sit amet.")}
    >
    More Info
    </button>
    </div>
